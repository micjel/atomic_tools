...
class Atom:
    ...
    def get_valence_occupation(self):
        """
        Get the valence occupation of the atom in terms of occupied single-orbital
        subshells. 
        Returns a list [c, n_s, n_p, n_d, n_f] where n_s, n_p, n_d, and n_f are the number
        of occupied single-orbital subshells in the s, p, d, and f shells respectively,
        and c is the number of electrons in the closed shell core. 
        """
        
        Nval = self.Ne - self.get_core()
        if self.Ne <= 20: # s and p shells only
            if Nval >= 2:
                n_s = 2
                n_p = 6 - Nval 
            else:
        ...
        if self.Ne <= 56:
            if Nval >= 12:
                ...
                n_p = 0
        n_f = 0
        return [self.get_core(), n_s, n_p, n_d, n_f]

    def get_valence_subshell(self):
        """
    ...
    def get_gs_Jp_term(self):
        ...
        highest_l = self.get_valence_subshell()
        print(highest_l)
...
class Snt(Atom):
    ...
    def get_snt_fname(self):
        return f'{self.shell}_{self.hamil}_{self.args}_{self.get_ref()}_{self.element}_e{self.emax}_s{self.s}_eta{self.zeta}.snt'
    
    def get_ion_snt_fname(self, q):
        return f'{self.shell}_{self.hamil}_{self.args}_{self.get_ion_ref(q)}_{self.element}_e{self.emax}_s{self.s}_eta{self.zeta}.snt'
    
    def get_summary_fname(self):
        return f'summary_{self.get_ref()}_{self.shell}_{self.hamil}_{self.args}_{self.get_ref()}_{self.element}_e{self.emax}_s{self.s}_eta{self.zeta}.txt'

    def get_ion_summary_fname(self, q):
        return f'''summary_{self.get_ion_ref()}_{self.shell}_{self.hamil}_{self.args}_{self.get_ion_ref(q)}_{self.element}_e{self.emax}_s{self.s}_eta{self.zeta}.txt'''
    
    def vspace_diag(self, path_to_kshell='', scratch=True):
